...
class Db(object):
    ...
    def __call__(self, m_templ, *m_args, **kw):
        ...
        if templ.lstrip().lower().startswith('select'):
...
class RecordSet(collections.Sequence):
    """
    A sequence of homogeneous records
    """
    ...
    def __repr__(self):
        return '<RecordSet%s, %d records>' % (self.fields, len(self.records))
...
class Record(collections.Sequence):
    """
    Record belonging to a RecordSet
    """
